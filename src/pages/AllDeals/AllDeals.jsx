...
import { FaClock, FaUsers, FaSearch, FaTags, FaFilter } from "react-icons/fa";
  ...
  return (
    <Link to={`/deals/${deal.id}`} className="group bg-white rounded-2xl shadow hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 flex flex-col">
      <div className="relative overflow-hidden h-48">
        ...
        <span className={`absolute top-3 left-3 text-xs font-semibold px-2.5 py-1 rounded-full shadow ${deal.status === "Active" ? "bg-emerald-100 text-emerald-700" : deal.status === "Successful" ? "bg-blue-100 text-blue-700" : "bg-red-100 text-red-600"}`}>
          {deal.status}
        </span>
        {product.brand && <span className="absolute bottom-3 left-3 bg-black/60 text-white text-xs px-2 py-0.5 rounded-full">{product.brand}</span>}
      </div>
      <div className="p-4 flex flex-col gap-2 flex-1">
        ...
        <div>
          <div className="flex justify-between text-xs text-gray-400 mb-1"><span>{deal.current_quantity} joined</span><span>{progress}%</span></div>
          <div className="w-full bg-gray-100 rounded-full h-2.5">
            <div className={`h-2.5 rounded-full transition-all ${progress >= 100 ? "bg-emerald-500" : "bg-gradient-to-r from-[#34699A] to-emerald-400"}`} style={{ width: `${Math.min(progress, 100)}%` }} />
          </div>
        </div>

        <CountdownDisplay endTime={deal.end_time} status={deal.status} />

        <button className="mt-auto w-full py-2.5 bg-gradient-to-r from-[#34699A] to-[#58A0C8] text-white text-sm font-semibold rounded-xl hover:opacity-90 transition">
          {deal.status === "Active" ? "Join Deal" : "View Details"}
        </button>
      </div>
    </Link>
  );
...
const AllDeals = () => {
  ...
  const fetchDeals = (q, f) => {
    setLoading(true);
    const url = q ? `/api/search?q=${encodeURIComponent(q)}` : `/api/deals${f !== "All" ? `?status=${f}` : ""}`;
  ...
  const categories = ["Grains & Rice", "Oils & Fats", "Beverages", "Dairy", "Snacks", "Cleaning Supplies"];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-[#34699A] to-[#58A0C8] py-12 px-6 text-white text-center">
        ...
        <p className="text-blue-100 text-sm">Join bulk deals and save big on Kuwait supermarket essentials</p>
        ...
        <form onSubmit={handleSearch} className="mt-6 flex max-w-xl mx-auto">
          <div className="flex items-center bg-white rounded-l-xl px-4 flex-1">
            ...
            <input value={searchInput} onChange={e => setSearchInput(e.target.value)} placeholder="Search products, brands, categories..." className="py-3 outline-none text-gray-700 w-full text-sm" />
          </div>
      ...
      <div className="max-w-[1300px] mx-auto px-6 py-8">
        <div className="flex flex-wrap items-center gap-3 mb-8">
          ...
          {["Active", "Successful", "Failed", "All"].map(s => (
            <button key={s} onClick={() => { setFilter(s); setQuery(""); setSearchInput(""); }}
              className={`px-4 py-1.5 rounded-full text-sm font-medium transition ${filter === s && !query ? "bg-[#34699A] text-white" : "bg-white border border-gray-200 text-gray-600 hover:border-[#34699A]"}`}>
              {s}
            </button>
          ))}
